# -*- coding: utf-8 -*-
"""
Created on Sun Feb  4 18:15:00 2024
...
parser.add_argument('--arch', '-a', metavar='ARCH', default='mnist_densenet',
                    choices=model_names,
...
parser.add_argument('--lam', default=5e-8, type=float, metavar='M',
                    help='lambda')
...
parser.add_argument('--gpu', default=None, type=int,
                    help='GPU id to use.')
parser.add_argument('--save', default='./results/model_data/densenet', type=str,
...
def main():
    ...
    if args.pretrained:
        ...
        model = models.__dict__[args.arch](depth=40,dataset='mnist')
    ...
    transform_val = transforms.Compose(
      [transforms.Pad(2), # Padding to a 32 x 32 image so the output dimensions after pooling
       transforms.ToTensor()])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        with open(os.path.join(args.save, 'densenet_mnist_results_lr%.4f_lam%.8f_mom%.6f.txt' % (args.lr, args.lam, args.momentum)), "a+") as text_file:
...
def save_checkpoint(state, is_best, checkpoint, args):
    filename = 'densenet_mnist_checkpoint_lr%.4f_lam%.8f_mom%.6f.pth.tar' % (
        args.lr, args.lam, args.momentum)
    filepath = os.path.join(checkpoint, filename)
    torch.save(state, filepath)
    if is_best:
        shutil.copyfile(filepath, os.path.join(
            checkpoint, 'densenet_mnist_model_best_lr%.4f_lam%.8f_mom%.6f.pth.tar' % (args.lr, args.lam, args.momentum)))
