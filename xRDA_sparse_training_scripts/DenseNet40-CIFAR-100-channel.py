...
parser.add_argument('--lam', default=0, type=float, metavar='M',
                    help='lambda')
...
def main():
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        with open(os.path.join(args.save, 'densenet_cifar100_results_lr%.4f_lam%.8f_mom%.6f.txt' % (args.lr, args.lam, args.momentum)), "a+") as text_file:
...
def save_checkpoint(state, is_best, checkpoint, args):
    filename = 'densenet_cifar100_checkpoint_lr%.4f_lam%.8f_mom%.6f.pth.tar' % (
        args.lr, args.lam, args.momentum)
    filepath = os.path.join(checkpoint, filename)
    torch.save(state, filepath)
    if is_best:
        shutil.copyfile(filepath, os.path.join(
            checkpoint, 'densenet_cifar100_model_best_lr%.4f_lam%.8f_mom%.6f.pth.tar' % (args.lr, args.lam, args.momentum)))
